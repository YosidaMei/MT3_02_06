...
bool isCollisionAABB(const AABB& aabb1, const AABB& aabb2) {
	...
	ab2 = aabb2;
	//ab1
	ab1.min.x = (std::min)(aabb1.min.x, aabb1.max.x);
	ab1.max.x = (std::max)(aabb1.min.x, aabb1.max.x);
	ab1.min.y = (std::min)(aabb1.min.y, aabb1.max.y);
	ab1.max.y = (std::max)(aabb1.min.y, aabb1.max.y);
	ab1.min.z = (std::min)(aabb1.min.z, aabb1.max.z);
	ab1.max.z = (std::max)(aabb1.min.z, aabb1.max.z);
	//ab2
	ab2.min.x = (std::min)(aabb2.min.x, aabb2.max.x);
	ab2.max.x = (std::max)(aabb2.min.x, aabb2.max.x);
	ab2.min.y = (std::min)(aabb2.min.y, aabb2.max.y);
	ab2.max.y = (std::max)(aabb2.min.y, aabb2.max.y);
	ab2.min.z = (std::min)(aabb2.min.z, aabb2.max.z);
	ab2.max.z = (std::max)(aabb2.min.z, aabb2.max.z);
	//衝突
	if ((ab1.min.x <= ab2.max.x && ab1.max.x >= ab2.min.x) &&
		(ab1.min.y <= ab2.max.y && ab1.max.y >= ab2.min.y) &&
		(ab1.min.z <= ab2.max.z && ab1.max.z >= ab2.min.z)) {
		return true;
	}
	else {
		return false;
...
bool isCollisionAB2S(const AABB& aabb, const Sphere& sphere) {
	//最近接点を求める
...
void DrawAABB(const AABB& aabb, const Matrix4x4& viewProjectionMatrix, const Matrix4x4& viewportMatrix, uint32_t color) {
	Vector3 vertex[8];
	vertex[0] = { aabb.min };
	vertex[1] = { aabb.min.x,aabb.max.y,aabb.min.z };
	vertex[2] = { aabb.max.x,aabb.max.y,aabb.min.z };
	vertex[3] = { aabb.max.x,aabb.min.y,aabb.min.z };
	vertex[4] = { aabb.max.x,aabb.min.y,aabb.max.z };
	vertex[5] = { aabb.min.x,aabb.min.y,aabb.max.z };
	vertex[6] = { aabb.min.x,aabb.max.y,aabb.max.z };
	vertex[7] = { aabb.max };
	for (int32_t index = 0; index < 8; ++index) {
		vertex[index] = Transform(Transform(vertex[index], viewProjectionMatrix), viewportMatrix);
	}
	Novice::DrawLine(vertex[0].x, vertex[0].y, vertex[1].x, vertex[1].y, color);
	Novice::DrawLine(vertex[1].x, vertex[1].y, vertex[2].x, vertex[2].y, color);
	Novice::DrawLine(vertex[2].x, vertex[2].y, vertex[3].x, vertex[3].y, color);
	Novice::DrawLine(vertex[0].x, vertex[0].y, vertex[3].x, vertex[3].y, color);
	Novice::DrawLine(vertex[1].x, vertex[1].y, vertex[6].x, vertex[6].y, color);
	Novice::DrawLine(vertex[6].x, vertex[6].y, vertex[7].x, vertex[7].y, color);
	Novice::DrawLine(vertex[2].x, vertex[2].y, vertex[7].x, vertex[7].y, color);
	Novice::DrawLine(vertex[4].x, vertex[4].y, vertex[7].x, vertex[7].y, color);
	Novice::DrawLine(vertex[4].x, vertex[4].y, vertex[5].x, vertex[5].y, color);
	Novice::DrawLine(vertex[5].x, vertex[5].y, vertex[6].x, vertex[6].y, color);
	Novice::DrawLine(vertex[0].x, vertex[0].y, vertex[5].x, vertex[5].y, color);
	Novice::DrawLine(vertex[3].x, vertex[3].y, vertex[4].x, vertex[4].y, color);
}
